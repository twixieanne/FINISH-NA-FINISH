$(document).ready(function () {
  ...
  $(".image-card").on("click", function () {
    ...
    $(".image-card-popover").not($(this).find(".image-card-popover")).fadeOut();
    $(this).find(".image-card-popover").fadeToggle();
  });

  $(document).on("click", function (e) {
    if (!$(e.target).closest(".image-card").length) {
      $(".image-card-popover").fadeOut();
    }
  });
});
...
$("#armbtn").click(function () {
  togglePopover('armorPopover');
});

$("#arbtn").click(function () {
  togglePopover('arrowPopover');
});

$("#swbtn").click(function () {
  togglePopover('swordPopover');
});
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function createParticle() {
    ...
    // Set a random position for each particle in the background
    const randomX = Math.random() * window.innerWidth;
    const randomY = Math.random() * window.innerHeight;
    particle.style.left = `${randomX}px`;
    particle.style.top = `${randomY}px`;

    // Remove the particle after the animation ends (8 seconds)
    setTimeout(() => {
      particle.remove();
    }, 8000); // Adjust time to match animation duration
  }

  // Generate particles every 300ms
